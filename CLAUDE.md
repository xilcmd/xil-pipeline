...
- Direction pipe-hints: `[<TEXT> | <file>.mp3 | <key>=<value>]` — segments after the direction text are classified independently (order-free, either half optional). A `.mp3`/`.wav` segment becomes the cue's `source`; a `key=value` segment whose key is in `HINT_ATTRS` becomes a per-cue config override (`play_volume_pct=20%` → `volume_percentage: 20`, range 0–200; `play_duration_pct=35%` → `play_duration: 35`, range 0–100; the `%` is optional on both). Unrecognized segments are left on the direction text rather than swallowed, and a malformed value warns and is dropped instead of failing the parse. Hints land on the parsed entry as `sfx_source` / `sfx_overrides`, then flow into `sfx_<TAG>.json` via `generate_sfx_config()` (new config) or `backfill_sfx_sources()` / `xil sfx-hydrate` (existing config). **A `source` hint never replaces one already in the config; attribute hints do overwrite** — the script is authoritative for playback settings. Silence cues (BEAT) ignore attribute hints, and looped layers (AMBIENCE, VINTAGE FILTER) drop `play_duration` with a warning since `mix_common` only honours it for MUSIC/SFX/BEAT. A `play_duration` hint also clears `duration_seconds` on **source-backed** cues (mutually exclusive; stops the skeleton's default 5.0 contradicting the hint) but never on generated cues, where `duration_seconds` is the API generation length. Both rules live in `filter_sfx_overrides()` / `_apply_sfx_overrides()` so the two write sites and `xil sfx-hydrate`'s report cannot drift apart. Add a key to `HINT_ATTRS` to support a new attribute; the rest of the chain is generic
...
- Supports `--seq-list "12,45,88,203"` (comma-separated, dialogue only) to regenerate an arbitrary, non-contiguous set of seqs in a single invocation — takes precedence over `--start-from`/`--stop-at` when given. This is the mechanism for touching up a handful of scattered lines: the TTS worker/subprocess (notably the local Chatterbox Turbo model) starts once and is reused across every listed seq, instead of paying the model-load cost once per `--start-from`/`--stop-at` invocation. Does not affect `--gen-sfx`/`--gen-music`/`--gen-ambience` filtering, which only understands `--start-from`. Combine with `--force` to overwrite stems already on disk; the dry-run summary label reads `SEQ-LIST 12,45,88,203`
...
- `mix_common.py` — shared mixing utilities (timeline, layer builders, fast label helpers) used by XILP003 and XILP005; `StemPlan.scene` (str|None): scene label from parsed JSON, used for scene-scoped vintage filter; `StemPlan.loop` field: `True` (default) tiles audio, `False` plays once up to scene boundary; `StemPlan.pre_trimmed` flag: skips play_duration trim for source-based stems already trimmed at copy time; `StemPlan.volume_percentage` (float|None): volume as a percentage (100 = unity, None = no change); `StemPlan.ramp_in_seconds` / `StemPlan.ramp_out_seconds`: fade durations in seconds (None = no fade); `_resolve_audio_params()` resolves volume/ramp from per-effect config or category defaults for MUSIC, AMBIENCE, SFX, and BEAT direction types; `volume_percentage`, `ramp_in_seconds`, and `ramp_out_seconds` each fall back to the global key when no category-specific key exists (e.g. SFX/MUSIC when `sfx_volume_percentage`/`music_ramp_in_seconds` are absent from the config defaults); `collect_stem_plans()` skips stale stems (header entries, type mismatch, speaker mismatch), deduplicates by seq number, and injects synthetic stop-marker `StemPlan` entries (filepath="") for `AMBIENCE: STOP` and `AMBIENCE: * FADES OUT` directives found in the entries index; `build_sfx_layer()` and `build_foreground()` apply `volume_percentage` to SFX/BEAT stems; `build_ambience_layer()` skips corrupt or unreadable stem files with a warning rather than crashing; `apply_vintage_filter()` applies a 1960s-era HF roll-off + 1 dB reduction; `_apply_speaker_filters(segment, filter_val)` resolves the cast config `filter` string and dispatches through `FILTER_REGISTRY` (name → module-level function name, resolved late so patching works): `phone`, `vintage`, `film`, `speakerphone`; unknown names warn once and pass through; only `vintage` is still pure pydub — `phone` was rebuilt as an ffmpeg treatment because pydub's single-pole filters left it only ~11 dB down at 80 Hz (now ~37 dB) and it added a flat +5 dB that made the caller louder than the room (now ~1.4 dB under, calibrated on real dialogue stems); `Treatment` carries optional `codec`/`container`/`codec_rate` fields and `run_ffmpeg_filter` round-trips through them via `_codec_round_trip()` after the filter pass (phone uses libgsm at 8 kHz in the raw `gsm` muxer — GSM is rejected inside a WAV container), all three folded into the cache key; a missing codec degrades to the filter-only result rather than the dry input, so `_fail()` takes a `consequence` string instead of always claiming the audio is untreated; `_span_treatments(stem_plans)` maps dialogue seq → ordered treatment names from open script spans, driven by `SPAN_DIRECTION_TREATMENTS` (`VINTAGE FILTER`→`vintage`, `FILM AUDIO`→`film`, `SPEAKERPHONE`→`speakerphone`, `PHONE FILTER`→`phone`); overlapping spans stack in declaration order; an ENGAGES marker may name a speaker (`[PHONE FILTER: ENGAGES DEZ]`) to narrow the span to that speaker's lines — parsed by `_span_scope()`, matched against `_plan_speaker()`, and a bare ENGAGES still treats every enclosed line; `_span_names_to_apply()` drops a span treatment listed in `_DEDUPED_TREATMENTS` (currently `phone` only) when the speaker's cast filter already applied it, since phone is a band-limit plus a fixed +5 dB and stacking it lands +10 dB — vintage/film keep their documented double-treatment; `missing_codecs(names)` reports which in-use treatments declare a codec this ffmpeg build lacks (probe cached per encoder), and `mix_common._warn_missing_treatment_codecs()` calls it from the top of both layer builders so a libgsm-less box warns before the first stem rather than mid-render — the failure is silent inconsistency between machines, not a crash; `_cast_filter_names()` is the single normalisation of the cast `filter` value (True→phone, comma chains, lowercase) shared by `_apply_speaker_filters` and the dedupe check; `_vf_engaged_seqs(stem_plans)` is a thin wrapper retained for the existing test surface — used by `build_foreground()` and `build_dialogue_layer()` (script-direction spans take precedence over `vintage_scenes` list fallback); `_SPAN_SENTINEL_MARKERS` controls which markers get synthetic boundary plans (VINTAGE FILTER: DISENGAGES only, since ENGAGES binds a real crackle stem; FILM AUDIO: both)
...
This is deliberately the opposite of the rule for attribute hints (`play_volume_pct`, `play_duration_pct` — #49, script wins). Durability across a script rebuild was the goal for `source`; immediate script authority was the goal for attributes. The warning is what stops the difference being silent.
