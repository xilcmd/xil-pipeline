...
| `play_duration_pct=35%` | Becomes the cue's `play_duration` (0–100; the `%` is optional). |
| anything else | Not consumed — re-joined onto the direction text, so a prose note survives verbatim. |

Segments are classified independently, so order does not matter and any part may
appear alone:
...
flowchart TD
    ...
    Audacity macro (WAVs only, Audacity 3)
...
> each WAV layer. Import labels separately via `File > Import > Labels...` in Audacity. This still works
> in Audacity 4, but **v4 does not name the imported track after the file** — import them in the order the
> helper script lists and rename each track, or they are indistinguishable.
...
> `Tools > Macros` in Audacity 3.
>
> **Audacity 4.0 removed Macro Manager**, so the macro cannot be run there. The macro is still written —
> v3 is often installed alongside — but `export_daw_layers()` warns when `detect_audacity_generations()`
> reports a v4 config. Detection reads `%APPDATA%\audacity\` (`Audacity4.ini`/`Audacity4/` for v4,
> `Macros/` for v3), so it means "has been run here", not "is installed now".
...
> (`--macro`) is the recommended automation path there.
>
> **Neither is available in Audacity 4.0** — it removed both Macro Manager and the scripting pipe
> (the changelog says they return in a later release). On v4 the pipe open fails, the helper script
> falls through to `print_instructions()`, and import is manual. That fallback already existed; only
> the wording changed.
...
> confirmed working; otherwise omit this flag and save manually. Inert on Audacity 4, which has
> no pipe and offers `.aup4` as well as `.aup3` on save.
...
| `--sfx PATH` | `configs/<slug>/sfx_<TAG>.json` | Override SFX config path. When the file exists, direction entries are emitted with a pipe-hint suffix (`[SFX: TEXT \| filename.mp3 \| play_volume_pct=20% \| play_duration_pct=35%]`) for any entry whose SFX config key has a `source`, a `volume_percentage`, and/or a `play_duration`. Bare-prompt and silence entries are unaffected. |
...
Cues carrying a `volume_percentage` or a `play_duration` also emit the matching
attribute hint, with or without a filename, so a regenerated script round-trips
losslessly:

```
[OUTRO MUSIC | sundy3M4_v3_(tech)_1.05mn_125bpm_-_TAGEO.mp3 | play_volume_pct=20%]
[MUSIC: STING | play_volume_pct=40%]
[INTRO MUSIC | The Porch Light.mp3 | play_volume_pct=40% | play_duration_pct=35%]
```
...
flowchart TD
    ...
    direction → [TEXT], [TEXT | file.mp3], + | play_volume_pct=N% | play_duration_pct=N%
