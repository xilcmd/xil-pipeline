...
def inject_postamble_entries(parsed_path: str, postamble_text: str, speaker: str) -> tuple[int, int]:
    """Append postamble voice + OUTRO MUSIC entries at the end of the parsed JSON.

    Idempotent: removes any existing ``section="postamble"`` entries before
    appending, then assigns seq = max_episode_seq + 1 (voice) and +2 (music).
    ...
    Returns:
        Tuple of (voice_seq, music_seq) — the assigned sequence numbers.
    """
    with open(parsed_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    ...
    data["entries"] = data["entries"] + postamble_entries
    with open(parsed_path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print(f"   Injected postamble entries (seq {voice_seq}, {music_seq}) into {parsed_path}")
    return voice_seq, music_seq
...
def main() -> None:
    ...
    if cast_cfg.postamble and os.path.exists(args.script):
        ...
        postamble_music_stem = os.path.join(stems_dir, f"{_max_seq + 2:03d}_postamble_sfx.mp3")
    ...
    if args.dry_run:
        ...
        if cast_cfg.postamble and postamble_text is not None and os.path.exists(args.script):
            os.makedirs(stems_dir, exist_ok=True)
            # inject first to get confirmed seq numbers, then generate stems
            voice_seq, music_seq = inject_postamble_entries(
            ...
            # Copy outro music from sfx config 'OUTRO MUSIC' source
            if not os.path.exists(postamble_music_stem):
                if sfx_config_model and "OUTRO MUSIC" in sfx_config_model.effects:
                    ...
                    print(" [!] No 'OUTRO MUSIC' entry in sfx config — skipping outro music stem")
